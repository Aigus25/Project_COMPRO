...
def generate_report():
    ...
    with open("report.txt", "w", encoding="utf-8") as f:
        ...
        sep = "-" * 110 + "\n"
        f.write(sep)
        f.write(f"| {'ID':<6} | {'Code':<10} | {'Title':<20} | {'Category':<12} | {'Credits':<7} | {'Fee':<9} | {'Status':<7} | {'Full':<5} | {'Instructor':<15} |\n")
        f.write(sep)
        for c in courses:
            ...
            f.write(f"| {c[0]:<6} | {decode_str(c[1])[:10]:<10} | {decode_str(c[2])[:20]:<20} | "
                     f"{decode_str(c[3])[:12]:<12} | {c[4]:<7} | {c[5]:<9.2f} | {st:<7} | {full:<5} | {instructor[:15]:<15} |\n")
        f.write(sep + "\n")
